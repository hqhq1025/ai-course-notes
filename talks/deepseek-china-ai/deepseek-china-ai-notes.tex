...
\newcommand{\videopublishdate}{2025-02-01}
\newcommand{\videoduration}{5小时06分钟}
\newcommand{\videourl}{https://www.youtube.com/watch?v=7FGonJBfv4s}
...
NVIDIA 股价在 DeepSeek 发布后暴跌，但这是基于错误叙事的社交传染。没有任何公开模型的训练花费超过 \$1B。真正的故事不是``模型变便宜了所以 GPU 不需要了''，而是 Jevons 悖论的完美演绎。

\begin{importantbox}{Jevons 悖论在 AI 中的验证}
经济学中的 Jevons 悖论：当一种资源的使用效率提高时，总消耗量反而增加（因为降低成本刺激了更多需求）。
...
更便宜的推理 $\to$ 更多的应用场景 $\to$ 更多的推理需求 $\to$ 需要更多 GPU。这不是零和博弈。
\end{importantbox}

\subsection{GPU 走私的规模与手段}

GPU 走私已经从灰色地带发展为产业级操作。

\textbf{租用渠道}：字节跳动是最大的``走私者''——从 Oracle、Google、各小型云商全球租用 500,000+ GPU。通过分散在多个国家的实体来规避单一审查。

\textbf{物理走私}：有人在旧金山机场头等舱带着 SuperMicro GPU 服务器箱子飞上海。2024 年估计有 200-300K GPU 通过新加坡、马来西亚的小公司流入中国。

\textbf{新扩散规则}：限制中国关联实体租用 $<$2000 GPU 或购买 $<$1500 GPU。但这创造了一个新的执法挑战——如何追踪全球云计算资源的最终用户？

\textit{``GPU 将超越毒品和武器成为每公斤最高价值的走私品。''} (Dylan) 一张 H100 售价约 \$30,000，重量不到 5kg。每公斤价值 \$6,000+，远超大多数违禁品。

\subsection{蒸馏、工业间谍与知识产权}

\subsubsection{蒸馏争议}

蒸馏——用更强模型的输出来训练弱模型——是行业标准做法。OpenAI 声称 DeepSeek 使用了他们的 API 输出，金融时报大标题报道。但真相更复杂：

\textit{``为什么我用你的模型输出训练是不道德的，而你可以用互联网的文字训练？''} (Nathan) 用户通过 ChatGPT 生成的内容被复制粘贴到互联网上，成为了公开可获取的数据。很多美国创业公司（不仅是中国公司）公开在 OpenAI 输出上训练。

\subsubsection{人才与想法的流动}

硅谷的跳槽文化（加州非竞争条款违法）使得想法自由流动。一个具体案例：帮助 Google 建造 Gemini 100 万 context 的工程师跳槽到 Meta——下一代 Llama 预期将有 100 万 context。

\begin{knowledgebox}{日本漏洞}
日本可能成为 AI 训练的``天堂''：
\begin{itemize}
    \item 版权法允许在任何数据上训练（明确的法律豁免）
    \item 9GW 搁置核电容量可重启用于数据中心
    \item 无 GPU 进口限制
    \item 多家日本公司已开始建设 AI 训练设施
\end{itemize}
\end{knowledgebox}

针对 AI 研究者的情报活动也在发生。Dylan 半开玩笑地指出：\textit{``作为一个二十多岁的单身男人……我们非常容易被腐蚀。''} 钓鱼行动（honey pot）针对拥有前沿知识的年轻研究者是真实存在的安全威胁。

\subsection{本章小结}

AI 产业的经济学正在经历极速重构：1200 倍的成本下降、Jevons 悖论推动需求增长、GPU 走私成为新的地下产业。知识产权边界模糊——蒸馏、人才流动、开源模型使得``谁拥有什么''变得极难定义。日本的法律框架可能使其成为意外的 AI 训练中心。
