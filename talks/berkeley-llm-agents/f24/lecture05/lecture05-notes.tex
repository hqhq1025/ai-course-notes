...
\newcommand{\notetitle}{LLM Agents MOOC | Compound AI Sys \& DSPy Framework}
\newcommand{\noteauthors}{基于 Omar Khattab 授课内容整理}
\newcommand{\notedate}{2024年10月9日}
\newcommand{\videochannel}{Berkeley RDI}
\newcommand{\videopublishdate}{2024年10月9日}
\newcommand{\videoduration}{1:04:58}
\newcommand{\videourl}{https://www.youtube.com/watch?v=JEMYuzrKLUw}
...
UC Berkeley CS294-196 Fall 2024 的第 5 课由 Omar Khattab 讲授，题为 ``Compound AI Sys \& DSPy Framework''，目的是阐释如何将多模态 LLM 代理与符号优化器组合。
...
Khattab 继续深入 Compound AI，第一步是把搜索过程本身视为监督信号，既保留了 LLM 的生成能力，又让结果具有可解释的结构。
...
Khattab 提出了 ``agent judge'' 的思想：不再依赖人工标注整条 trace，而是用经过 DPO fine-tune 的 agent 作为 judge，对每一次互动的 plan 进行打分，模仿 human preference 并进一步优化 generation。
\begin{table}[H]
\centering
...
Omar Khattab 多次提醒：Compound AI 的核心不是再堆一个大模型，而是把有限的资源用在多个环节。我们把资源拆成三个层面：search trace 的采样成本、APAC 多轮对话的实时 cost、和 DSPy solver 的 gradient 更新。
...
\subsection{拓展阅读}
\begin{itemize}
    \item Khattab et al., ``Search Former'' (2024)，在 Search Former 上加入 drop 层学习更紧凑的 trace。 
    \item Khattab et al., ``DU Forer'' (2024)，通过 trace drop 实现不同粒度的 trace reconstruction。
