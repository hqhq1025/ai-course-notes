...
$$\text{RLHF（优化）: } \text{找到 } \hat{p}(y|x) \text{ 使得 } \max_p \mathbb{E}_p[R(y,x)]$$
...
$$P(y_A \succ y_B | x) = \sigma(R(y_A, x) - R(y_B, x))$$
...
$$\text{objective}(\phi) = \mathbb{E}_{(x,y) \sim D_{\pi^{\text{RL}}_\phi}} \left[ r_\theta(x, y) - \beta \log \frac{\pi^{\text{RL}}_\phi(y|x)}{\pi^{\text{SFT}}(y|x)} \right] + \gamma \mathbb{E}_{x \sim D_{\text{pretrain}}} \left[ \log \pi^{\text{RL}}_\phi(x) \right]$$
...
$$\nabla_\theta \mathbb{E}_{y \sim p_\theta}[R(y)] = \mathbb{E}_{y \sim p_\theta}[R(y) \cdot \nabla_\theta \log p_\theta(y)]$$
